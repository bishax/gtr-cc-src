...
def main(train_size, test_size, target, random_state):
    ...
    Xy = (read_csv(f"{data_dir}/processed/gtr_tokenised.csv", usecols=[target])
          .join(read_csv(f"{data_dir}/processed/gtr_embedding.csv", index_col=0))
          )
    ...
    train_fout = f"{data_dir}/processed/gtr_train.csv"
    test_fout = f"{data_dir}/processed/gtr_test.csv"
